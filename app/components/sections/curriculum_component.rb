...
class Sections::CurriculumComponent < ApplicationComponent
  ...
  def initialize(sections:, fallback_video: nil)
    @sections = sections
  ...
  attr_reader :sections, :fallback_video

  # Source for a lecture's preview modal: the lecture's own video when uploaded,
  # otherwise the shared course-preview (hero) video. nil if neither exists.
  def preview_url_for(lecture)
    video = lecture.video.attached? ? lecture.video : fallback_video
